...
ValueVariant::ValueVariant( const ValueVariant& other )
{
    Copy( other );
}

ValueVariant& ValueVariant::operator=( const ValueVariant& other )
{
    Copy( other );
    return *this;
}

ValueVariant::ValueVariant( ValueVariant&& other ) noexcept
{
    Move( std::move( other ) );
}

ValueVariant& ValueVariant::operator=( ValueVariant&& other ) noexcept
{
    Move( std::move( other ) );
    return *this;
}

ValueVariant::ValueVariant( int32_t iValue )
{
    SetInteger( iValue );
}

ValueKind ValueVariant::GetKind() const
{
    return mKind;
}

int32_t ValueVariant::GetInteger() const
{
    assert( mKind == ValueKind::Integer );
    return mValue.Integer;
}

std::shared_ptr<Function> ValueVariant::GetFunction() const
{
    assert( mKind == ValueKind::Function );
    return mValue.Function;
}

void ValueVariant::SetInteger( int32_t value ) noexcept
{
    Destroy();
...
void ValueVariant::SetFunction( const std::shared_ptr<Function>& function ) noexcept
{
    Destroy();
...
void ValueVariant::SetFunction( std::shared_ptr<Function>&& function ) noexcept
{
    Destroy();

    new (&mValue.Function) std::shared_ptr<Function>( std::move( function ) );
    mKind = ValueKind::Function;
}

void ValueVariant::Copy( const ValueVariant& other ) noexcept
{
    switch ( other.GetKind() )
    {
    case ValueKind::Integer: SetInteger( other.GetInteger() ); break;
    case ValueKind::Function: SetFunction( other.GetFunction() ); break;
    }
}

void ValueVariant::Move( ValueVariant&& other ) noexcept
{
    switch ( other.GetKind() )
    {
    case ValueKind::Integer:
        SetInteger( other.mValue.Integer );
        break;

    case ValueKind::Function:
        SetFunction( std::move( other.mValue.Function ) );
        break;
    }
...
void ValueVariant::Destroy() noexcept
{
