    ...
    case DeclKind::Const:
        {
            ...
            if ( IsKind<ValueKind::Integer>( constant->Value ) )
                EmitLoadConstant( GetByKind<ValueKind::Integer>( constant->Value ) );
            else
                EmitLoadFuncAddress( GetByKind<ValueKind::Function>( constant->Value ).get() );
        }
        break;
